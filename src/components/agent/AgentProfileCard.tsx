...
export default async function AgentProfileCard({ agentId, dict, locale }: { agentId: string, dict: any, locale: string }) {
    ...
    return (
        <div className="bg-white rounded-[2rem] p-5 sm:p-6 md:p-8 shadow-xl border border-slate-100 flex-shrink-0">
            <h3 className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-widest mb-6 text-center sm:text-left">{dict.common.agent_label}</h3>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 text-center">
                <Link href={`/${locale}/agents/${agentId}`} className="text-[10px] font-black text-navy-primary hover:text-indigo-600 transition-colors uppercase tracking-widest inline-flex items-center gap-1 group">
