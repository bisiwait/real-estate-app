...
import { Crown, ExternalLink, Loader2, CalendarX2 } from 'lucide-react'
...
export default function SubscriptionStatus({ profile }: SubscriptionStatusProps) {
    ...
    const isCancelScheduled = profile.auto_renew === false

    const bgColor = isCrisis ? 'bg-orange-50' : 'bg-white'
    const borderColor = isCrisis ? 'border-orange-200' : 'border-slate-100'
    const iconBg = isCrisis ? 'bg-orange-500' : 'bg-amber-400'
